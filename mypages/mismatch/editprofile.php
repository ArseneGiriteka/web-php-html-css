...
<?php
  ...
  if (isset($_POST['submit'])) {
    ...
    if (!empty($new_picture)) {
      if ((($new_picture_type == 'image/gif') || ($new_picture_type == 'image/jpeg') || ($new_picture_type == 'image/pjpeg') ||
        ...
        if ($_FILES['new_picture']['error'] == 0) {
          ...
          $target = MM_UPLOADPATH . basename($_FILES['new_picture']['name']);
      ...
      if (!empty($first_name) && !empty($last_name) && !empty($gender) && !empty($birthdate) &&         !empty( $city) && !empty($state)) {
        ...
        if (!empty($new_picture)) {
          $query = "UPDATE mismatch_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
            " birth_date = '$birthdate', city = '$city', state = '$state', picture = '$new_picture'".     " WHERE user_id = '".$_SESSION['user_id']."'";
        }
        else {
          $query = "UPDATE mismatch_user SET first_name = '$first_name', last_name = '$last_name', gender = '$gender', " .
            " birth_date = '$birthdate', city = '$city', state = '$state' WHERE user_id = '".           $_SESSION['user_id']."'";
        }
    ...
    $query = "SELECT first_name, last_name, gender, birth_date, city, state, picture FROM mismatch_user". " WHERE user_id = '".$_SESSION['user_id']."'";
...
?>
  ...
  <hr>
  <?php echo '<a href="questionnaire.php">Questionnaire</a><br />'; ?>
</body>
</html>
