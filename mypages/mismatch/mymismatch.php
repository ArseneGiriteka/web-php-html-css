<?php
  ...
  if (mysqli_num_rows($data) != 0) {
    ...
    $query = "SELECT mr.reponse_id, mr.topic_id, mr.reponse, mt.name AS topic_name " .
    ...
    while ($row = mysqli_fetch_array($data)) {
      ...
      $query2 = "SELECT reponse_id, topic_id, reponse FROM mismatch_reponse WHERE user_id = '" . $row['user_id'] . "'";
      $data2 = mysqli_query($dbc, $query2);
      $mismatch_reponses = array();
      while ($row2 = mysqli_fetch_array($data2)) {
        array_push($mismatch_reponses, $row2);
      }
      ...
      for ($i = 0; $i < count($user_reponses); $i++) {
        if ((int)$user_reponses[strval($i)]['reponse'] + (int)$mismatch_reponses[strval($i)]['reponse'] == 3) {
          $score += 1;
          array_push($topics, $user_reponses[(string)$i]['topic_name']);
        }
      }
